...
func (server *Server) createAccount(ctx *gin.Context) {
	...
	arg := db.CreateAccountParams{
		...
		Balance:  0, // Default balance to 0 for new accounts
...
type getAccountRequest struct {
	ID int64 `uri:"id" binding:"required,min=1"` // Ensure ID is a positive integer
}
...
type listAccountRequest struct {
	...
	PageSize int32 `form:"page_size" binding:"required,min=5,max=20"` // Ensure page size is within a reasonable range
}
